...
import (
	...
	"github.com/truong-devops/aiops-multiagent-rag-k8s/services/live-service/internal/config"
	"github.com/truong-devops/aiops-multiagent-rag-k8s/services/live-service/internal/handler"
...
func main() {
	...
	if err != nil {
		logger.Error("failed to initialize live store", "service", serviceName, "error", err)
		os.Exit(1)
	}
	defer closeStore()

	liveService := service.NewLiveService(store, service.Options{
		Metrics:         metrics,
		Logger:          logger,
		DefaultLimit:    cfg.LiveDefaultLimit,
		MaxLimit:        cfg.LiveMaxLimit,
		IngestBaseURL:   cfg.IngestBaseURL,
		PlaybackBaseURL: cfg.PlaybackBaseURL,
		StreamKeyBytes:  cfg.StreamKeyBytes,
	})

	mux := http.NewServeMux()
	handler.New(liveService).RegisterRoutes(mux, metrics.Handler())
...
func openStore(ctx context.Context, cfg config.Config, logger *slog.Logger, metrics *observability.Metrics) (repository.Store, func(), error) {
	if cfg.UsePostgres() {
		...
		logger.Info("using postgres live store", "service", serviceName)
		instrumented := repository.NewInstrumentedStore(store, metrics)
		return instrumented, func() {
			if err := store.Close(); err != nil {
				logger.Error("failed to close live store", "service", serviceName, "error", err)
			}
		}, nil
	}

	logger.Warn("using in-memory live store; this is only suitable for local development", "service", serviceName, "environment", cfg.Environment)
