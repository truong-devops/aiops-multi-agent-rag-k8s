...
Có, nhưng không nên làm quá sớm thành một service phức tạp.
...
để route traffic vào các service.

Khi sản phẩm bắt đầu có nhiều service và cần chuẩn hóa auth/rate limit/CORS, nên thêm API Gateway đúng nghĩa:

```text
Client
-> API Gateway / Ingress
-> identity-service, video-service, feed-social-service, live-service, aiops-service
```
...
/api/feed/*       -> feed-social-service
/api/live/*       -> live-service
/api/incidents/*  -> aiops-service
```
